...
- `~/Documents/EnglishPaperReader Library` を library folder として設定
...
| ☁️ **Data-only GitHub Sync** | `.paperapp/backup.sql` だけを GitHub に同期 |
...
語彙データは library folder 内の `.paperapp/backup.sql` に保存され、GitHub にはそのファイルだけを push します。
...
開発用のコード repo と混ざらないように、library folder は別ディレクトリにして使う想定です。
